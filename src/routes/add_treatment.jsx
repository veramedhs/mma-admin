...
const Add_treatment = () => {
  const { formData, loading, setFormData, createTreatment, resetForm } = useTreatmentStore();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // The form is automatically reset within the store on a successful submission.
    } catch (error) {
      // The store handles showing the toast error, so we just log it here.
      ...
      <div className="w-full max-w-4xl mx-auto bg-white dark:bg-slate-800 rounded-xl shadow-lg p-6 sm:p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <fieldset>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="md:col-span-2">
                ...
                <input id="name" type="text" name="name" value={formData.name} onChange={handleChange} placeholder="e.g., Total Knee Replacement" className={inputClass} disabled={loading} required />
              </div>
              <div className="md:col-span-2">
                <label htmlFor="summary" className={labelClass}>Summary / Description</label>
                <textarea id="summary" name="summary" value={formData.summary} onChange={handleChange} placeholder="A brief description of the treatment..." className={`${inputClass} min-h-[100px]`} disabled={loading} />
              </div>
            </div>
          </fieldset>
          ...
          <fieldset>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div>
                ...
                <input id="currency" type="text" name="currency" value={formData.currency} onChange={handleChange} placeholder="e.g., USD" className={inputClass} disabled={loading} />
              </div>
              <div>
                <label htmlFor="discountPercent" className={labelClass}>Discount (%)</label>
                <input id="discountPercent" type="number" name="discountPercent" value={formData.discountPercent} onChange={handleChange} placeholder="e.g., 10" className={inputClass} min="0" max="100" disabled={loading} />
              </div>
            </div>
          </fieldset>
          ...
          <fieldset>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input id="symptoms" type="text" name="symptoms" value={formData.symptoms} onChange={handleChange} placeholder="e.g., Severe pain, Stiffness" className={inputClass} disabled={loading} />
              </div>
              <div>
                <label htmlFor="tests" className={labelClass}>Required Tests (comma-separated)</label>
                <input id="tests" type="text" name="tests" value={formData.tests} onChange={handleChange} placeholder="e.g., X-ray, MRI Scan" className={inputClass} disabled={loading} />
              </div>
              <div>
                <label htmlFor="precautions" className={labelClass}>Precautions (comma-separated)</label>
                <input id="precautions" type="text" name="precautions" value={formData.precautions} onChange={handleChange} placeholder="e.g., Physical therapy, Use walkers" className={inputClass} disabled={loading} />
              </div>
              <div>
                <label htmlFor="tags" className={labelClass}>Search Tags (comma-separated)</label>
                <input id="tags" type="text" name="tags" value={formData.tags} onChange={handleChange} placeholder="e.g., knee surgery, arthritis" className={inputClass} disabled={loading} />
              </div>
            </div>
          </fieldset>
          ...
          <div className="flex flex-col-reverse sm:flex-row justify-end items-center gap-4 pt-4">
            <button type="button" onClick={resetForm} className="w-full sm:w-auto inline-flex justify-center items-center px-6 py-3 border border-slate-300 dark:border-slate-600 text-base font-medium rounded-md shadow-sm text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-700 hover:bg-slate-50 dark:hover:bg-slate-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-500 dark:focus:ring-offset-slate-800 transition-colors" disabled={loading}>
              Reset Form
            </button>
